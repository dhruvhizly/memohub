...
// const GRID_SIZES = {
//   Comfort: "columns-2 md:columns-4 lg:columns-6",
//   Compact: "columns-3 md:columns-5 lg:columns-8",
//   Dense: "columns-4 md:columns-6 lg:columns-10",
// };

// const GRID_SIZES_PROP = {
//   Comfort: "(max-width: 768px) 50vw, (max-width: 1024px) 25vw, 17vw",
//   Compact: "(max-width: 768px) 33vw, (max-width: 1024px) 20vw, 13vw",
//   Dense: "(max-width: 768px) 25vw, (max-width: 1024px) 17vw, 10vw",
// };
...
//                 Upload some files and thy will show up here.
//               </p>
//             </div>
//           ) : (
...
//                 </h2>
//                 <div
//                   className={`w-full ${GRID_SIZES[gridCols]} space-y-4 gap-4`}
//                 >
//                   {items.map((item) => {
...
//                         }
//                         className={`relative w-full overflow-hidden rounded-xl bg-neutral-900 border transition-all duration-200 cursor-pointer group break-inside-avoid mb-4 ${
//                           isItemSelected
...
//       <button
//         onClick={scrollToTop}
//         className={`fixed bottom-6 right-6 p-4 bg-gray-800 hover:bg-blue-500 text-white rounded-full shadow-2xl transition-all duration-300 z-50 cursor-pointer ${
//           showScrollTop
...
// --- DYNAMIC BREAKPOINTS ---
// We define how many columns to show at specific window widths
...
const SkeletonItem = ({ height }: { height: number }) => (
  <div
    ...
    className="w-full bg-neutral-900 border border-neutral-800 rounded-xl animate-pulse mb-4 relative overflow-hidden"
  >
    <div className="absolute inset-0 -translate-x-full animate-[shimmer_2s_infinite] bg-linear-to-r from-transparent via-neutral-800/30 to-transparent" />
  </div>
);
...
const GallerySkeleton = ({
  ...
  // We mimic the Masonry layout using Flex for the skeleton to prevent layout shift
  // We grab the approximate column count for a desktop view to render the skeleton
  const colCount = BREAKPOINT_MAPPING[mode].default;

  return (
    <div className="flex w-full gap-4">
      {Array.from({ length: colCount }).map((_, colIndex) => (
        <div key={colIndex} className="flex flex-col gap-4 w-full">
          {/* Generate random heights for each column to look like masonry */}
          {Array.from({ length: 3 }).map((_, itemIndex) => {
            // Deterministic random height based on index
            const height = 200 + (((colIndex + itemIndex) * 50) % 200);
